...
def codes_cracker(key):
    ...
    for link in links:
        ...
        if key.casefold() in href.casefold() and "index" not in title:
            study_materials[title] = "https://codescracker.com" + href
...
def coding_bat(key):
    try:
        ...
        for link in links:
            ...
            title = title.replace('/', ' ')
            if key.casefold() in href.casefold():
                study_materials[title] = "https://codingbat.com" + href
    except:
        pass
...
def guru(key):
    try:
        ...
        for link in links:
            title = link['href'][1:-5]
            if key in title:
                study_materials[title] = 'http://www.guru99.com' + link['href']
    except:
        pass
...
def tut_plus(key):
    try:
        ...
        for i in range(5, len(title)-10):
            study_materials[title[i]] = href[i]
    except:
        pass
def tech_gig(key):
    try:
...
def ws_vincent(key):
    ...
    for i in range(5, len(link_arr) - 1):
        books[link_arr[i][22:].replace('/', '')] = link_arr[i]
    ...
    try:
        ...
        for links in link:
            ...
            if key.casefold() in href:
                study_materials[href[16:-1]] = "https://frontendmasters.com" + href
    except:
        pass
...
def tutorials_in_hand(key):
    ...
    if res.status_code != 404:
        tutorials[url[38:-5].replace('/', ' ')] = url
...
def dream_host(key):
    ...
    for links in link:
        ...
        if 'help.dreamhost' in str(links.get('href')):
            ls = href.split('&')
            link_href = ls[0][7:]
            articles[link_href[55:]] = link_href